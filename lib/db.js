...
const FALLBACK_DB_FILE = path.join(process.cwd(), "data", "fallback-db.json");
...
export async function uploadAppFile(userId, fileName, fileBuffer, contentType) {
  ...
  const uploadsDir = path.join(process.cwd(), "public", "uploads", "fallback", userId);
  if (!fs.existsSync(uploadsDir)) {
    fs.mkdirSync(uploadsDir, { recursive: true });
  }
  const localPath = path.join(uploadsDir, `${Date.now()}-${fileName}`);
  fs.writeFileSync(localPath, fileBuffer);
  return { path: localPath, publicUrl: `/${path.relative(path.join(process.cwd(), "public"), localPath).replace(/\\/g, "/")}` };
}
