...
async function getWorkbook() {
  ...
  try {
    // Prefer using a writable temp copy in runtime (serverless deployments are often read-only)
    if (fs.existsSync(TEMP_FILE)) {
      await workbook.xlsx.readFile(TEMP_FILE);
    } else if (fs.existsSync(filePath)) {
      try {
        // Copy the read-only source to a writable temp location
        fs.copyFileSync(filePath, TEMP_FILE);
        await workbook.xlsx.readFile(TEMP_FILE);
      } catch (copyErr) {
        console.error("Failed to copy Excel to temp file:", copyErr.message);
        // Fall back to reading directly if copy fails
